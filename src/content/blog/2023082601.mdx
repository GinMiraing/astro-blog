...
description: 最近在公司写项目时，发现有一些可复用的代码没有封装，好多组件中都使用了后端传回的站点数据，这个数据是相同的，但是每个组件在使用这个数据之前都向后端接口发送了请求，明明只要一个请求就能搞定，现在的结果是产生了许多不必要的请求。通过查阅 React 官方文档，我发现使用 Context 可以只请求一次站点数据，并提供给所有组件使用。
author: 胤
category: 技术
id: 19
---
